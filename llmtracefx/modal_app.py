...
image = (
    ...
    .add_local_dir("./llmtracefx", remote_path="/app/llmtracefx", copy=True)
    .workdir("/app")
)
...
def analyze_trace_modal(
    ...
    try:
        ...
        if enable_claude and claude_api_key:
            ...
            async def generate_explanations() -> None:
                tasks = []
                ...
                for i, result in enumerate(results):
                    if isinstance(result, BaseException):
                        ...
                        explanations[analyses[i].token_id] = (
                            explainer.format_explanation_for_display(result)
                        )
        ...
        for analysis in analyses:
            bottleneck_summary[analysis.bottleneck_type] = (
                bottleneck_summary.get(analysis.bottleneck_type, 0) + 1
            )
...
@app.function(image=image, timeout=120, secrets=[Secret.from_name("claude-api-key")])
def explain_token_modal(token_analysis_data: dict[str, Any]) -> dict[str, Any]:
    """
    ...
    # Import local modules
    from llmtracefx.explainer.claude import ClaudeExplainer, ClaudeExplanation
    from llmtracefx.profiler.gpu_analyzer import GPUMetrics, TokenAnalysis
    ...
    try:
        ...
        gpu_metrics = GPUMetrics(
            ...
            memory_bandwidth_gb_s=token_analysis_data["gpu_metrics"][
                "memory_bandwidth_gb_s"
            ],
            compute_utilization=token_analysis_data["gpu_metrics"][
                "compute_utilization"
            ],
            occupancy_label=token_analysis_data["gpu_metrics"].get(
                "occupancy_label", "SM occupancy"
            ),
            metrics_source=token_analysis_data["gpu_metrics"].get(
        ...
        if claude_api_key:
            ...
            async def get_explanation() -> ClaudeExplanation:
                return await explainer.explain_token_performance(analysis)

            explanation = asyncio.run(get_explanation())
            formatted_explanation = explainer.format_explanation_for_display(
                explanation
            )

            return {"explanation": formatted_explanation, "status": "completed"}
        else:
            return {"error": "Claude API key not available", "status": "error"}
...
def main(
    trace_file: str = "test_traces/sample_vllm_trace.json", gpu_type: str = "A10G"
) -> None:
    """
